...
| Content portfolio / launch sequencing / first-contact category signal | **SPECIALIZATION + CAPABILITY** | Personal Brand Strategy specialization -> Content Portfolio & Launch Sequencing -> Content Architecture v0.4 | Own portfolio-level communication jobs: which evidence source plays which role, first-contact category/perception risk, launch-batch composition and sequence logic. Content Architecture remains artifact-level and must receive the asset role rather than inventing it. Current PBGS-IG-001 incident proves an applied gap but not yet a reusable new profession. | **NO SEND now.** Escalate upstream only if repeated cross-project evidence shows a reusable EXTEND/BUILD NEW delta. |
| Personal-brand naming / verbal identity / social `@username` | **CAPABILITY** with **REUSE** inputs + **DETERMINISTIC TOOL** checks; Brand Identity Agent as naming owner = **REJECT** | Positioning/strategy supplies identity constraints; Market & Competitive Intelligence supplies current category/competitor evidence; naming capability owns name/handle generation, linguistic/semantic screening, memorability/distinctiveness judgment and shortlist rationale; deterministic checks own handle/domain availability and exact-string collision checks; legal trademark clearance remains specialist/human/legal territory | Do not create a standalone Naming Agent/core for this one applied need. Treat naming/verbal identity as a modular capability in the brand/strategy layer. The Spline `brand-identity-agent` is project-specific visual-identity work, has no naming ownership, and has no transferable qualification evidence for this target. Until the capability is encoded/evaluated, do not present improvised username suggestions as professional naming output. | **NO SEND** now. Escalate upstream only if repeated cross-project evidence shows a stable reusable professional behavior delta that qualifies as EXTEND or BUILD NEW. |
...
- `personal-brand-growth-system/decisions/content-portfolio-launch-sequencing-gap-2026-09-13.md`
...
- repeated launch/portfolio decisions across projects show that Content Portfolio & Launch Sequencing requires a reusable professional behavior delta beyond the current Personal Brand Strategy specialization.
